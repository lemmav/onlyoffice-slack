...
class SettingsValidationServiceTests {
  @Mock OkHttpClientPoolService httpClientPoolService;
  @Mock JwtManagerService jwtManagerService;
  @InjectMocks SettingsValidationService settingsValidationService;
  ...
  void whenValidateConnectionWithValidRequest_thenNoExceptionThrown() throws IOException {
    ...
    doReturn("token")
        .when(jwtManagerService)
        .createToken(any(DocumentServerCommand.class), anyString());
    assertDoesNotThrow(() -> settingsValidationService.validateConnection(request));
  }
  ...
  void whenValidateConnectionWithInvalidAddress_thenThrowException() {
    ...
    assertThrows(Exception.class, () -> settingsValidationService.validateConnection(request));
  }
